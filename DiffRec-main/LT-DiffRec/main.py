...
parser.add_argument('--dataset', type=str, default=dataset_name, help='choose the dataset')
parser.add_argument('--data_path', type=str, default=f'../datasets/{dataset_name}/', help='load data path')
...
for epoch in range(1, args.epochs + 1):
    ...
    if epoch % 5 == 0:
        ...
        if valid_results[1][1] > best_recall:  # recall@20 as selection
            ...
            model_filename = f"{save_path}/model_{timestamp}.pth"
            ae_filename = f"{save_path}/autoencoder_{timestamp}.pth"

            torch.save(model, model_filename)
            torch.save(Autoencoder, ae_filename)
